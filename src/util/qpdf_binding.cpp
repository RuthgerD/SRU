...
Qpdf::decompress(std::filesystem::path pdf_file) const {
    const auto abs_pdf_file =
        std::filesystem::absolute(pdf_file.lexically_normal());
    auto abs_cache_path = std::filesystem::absolute(getCachePath());
    abs_cache_path.append(pdf_file.filename().generic_string());

    auto command = bin + " --stream-data=uncompress \"" +
                   abs_pdf_file.generic_string() + "\" -- \"" +
