...
PdfFile::PdfFile(std::filesystem::path path) : path{path} {
    ...
    int page_no = 0;
    if (auto page_matches = sru::util::re_search(sru::util::page_match_key, contents.value())) {
        for (auto page_match : page_matches.value()) {
            ...
            for (const auto pconf : sru::pdf::PageConfigPool) {
                if (auto result = sru::util::re_search(pconf.regex_id, page_match.at(2))) {
                    ...
                    timepool += (std::chrono::duration_cast<std::chrono::microseconds>(end - start).count()) / 1000000.0;
                    break;
                }
            }
        }
